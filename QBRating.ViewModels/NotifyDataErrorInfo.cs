...
namespace QBRating.ViewModels {
	...
	/// from https://rehansaeed.com/model-view-viewmodel-mvvm-part4-inotifydataerrorinfo/
	/// </summary>
	/// <typeparam name="T">The type of this instance.</typeparam>
	public abstract class NotifyDataErrorInfo<T> : ViewModelBase, INotifyDataErrorInfo
		...
		public bool NonNegWholeIntField(int? field) {
			return !field.HasValue || (field.HasValue && field >= 0);
		}
		public bool NotLargerThanDependentField(int? field, int? dependent) {
			return !field.HasValue || !dependent.HasValue || (field.HasValue && dependent.HasValue && field.Value <= dependent);
		}
		public bool FieldLessThan99TimesDependent(int? field, int? dependent) {
			if (!field.HasValue || !dependent.HasValue) { return true; }
			var max = dependent * 99;
		...
		protected override bool SetProperty<T>(ref T currentValue, T newValue, [CallerMemberName] string propertyName = null) {
			var result = base.SetProperty(ref currentValue, newValue, propertyName);
			if (result) {
